...
# Pangenome-aware variant calling: identical to germline-pangenome-cpu.cwl
# except that the five DeepVariant steps call with the graph's haplotypes as
# extra evidence (run_pangenome_aware_deepvariant), instead of from the reads
# alone. Mapping is unchanged -- vg giraffe still does it, and the caller only
# reads the same `gbz` the aligner used; no re-mapping and no giraffe indexes.
#
# The pangenome-aware binaries ship in their own image, which carries no plain
# run_deepvariant, so this workflow needs the image built by
# sif-build-pangenome-aware.def rather than the one for the CPU workflow.
#
# Its outputs do NOT match germline-pangenome-cpu.cwl record for record: a
# different model is used, the GQs differ and there is no MID field, so the
# reference outputs are kept separately (tests/*/demo_out/pangenome_aware/).

class: Workflow
id: germline-pangenome-pangenome-aware-cpu
label: germline-pangenome-pangenome-aware-cpu
cwlVersion: v1.1
...
inputs:
  ...
  sample_name_pangenome:
    type: string
    doc: Name recorded for the haplotype panel taken from the GBZ. Only cosmetic, but the tool's own default (hprc_v1.1) is wrong for any other graph.
    default: "GRCh38"
